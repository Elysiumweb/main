    ...
    <div className="grid lg:grid-cols-12 gap-10">
      <form onSubmit={submit} className="lg:col-span-5 space-y-4 border border-white/10 bg-[#1A1A1A] p-6" data-testid="admin-campaigns-form">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-[#f7f7f7]/50 block mb-1">{t("admin.campaigns.goal")}</label>
          ...
          <div>
            <label className="text-[10px] uppercase tracking-[0.2em] text-[#f7f7f7]/50 block mb-1">{t("admin.campaigns.current")}</label>
      ...
      <div className="lg:col-span-7 space-y-3" data-testid="admin-campaigns-list">
        {campaigns.length === 0 && <p className="text-[#f7f7f7]/40">{t("competitions.empty")}</p>}
        {campaigns.map((c) => (
          <div key={c.id} className="border border-white/10 bg-[#1A1A1A] px-5 py-4" data-testid={`admin-campaign-${c.id}`}>
            <div className="flex items-center gap-3">
              ...
              <span className={`text-[9px] uppercase tracking-widest border px-1.5 py-0.5 ${c.active ? "border-emerald-300/40 text-emerald-300" : "border-white/20 text-[#f7f7f7]/40"}`}>
                {c.active ? t("admin.campaigns.active") : t("status.closed")}
              </span>
              <button onClick={() => toggleActive(c)} className="text-[10px] uppercase tracking-wider text-[#f7f7f7]/50 hover:text-[#D8CA82]">{c.active ? "Désactiver" : "Activer"}</button>
