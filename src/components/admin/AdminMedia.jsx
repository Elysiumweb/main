...
export const AdminMedia = () => {
  ...
  const del = async (id) => { await deleteDoc(doc(db, "media", id)); };
    ...
    <div className="grid lg:grid-cols-12 gap-10">
      <form onSubmit={submit} className="lg:col-span-5 space-y-4 border border-white/10 bg-[#1A1A1A] p-6" data-testid="admin-media-form">
        ...
        {form.type === "photo" && form.url && isUrl(form.url) && (
          <BrandImage src={form.url} alt="Aperçu du média" ratio={RATIOS.card} className="w-full max-w-xs border border-white/10" />
        )}
        {form.type === "video" && (
        ...
        <ActionButton type="submit" variant="primary" size="md" loading={saving} loadingLabel="Enregistrement…"
          disabled={!form.title.trim() || !form.url.trim()} disabledReason="Le titre et l'URL sont obligatoires" data-testid="admin-media-submit">
          {t("notes.save")}
        </ActionButton>
      </form>
      <div className="lg:col-span-7 space-y-2" data-testid="admin-media-list">
        {media === null ? (
          <SkeletonList count={5} testId="admin-media-loading" label={t("common.loading")} />
        ) : media.length === 0 ? (
          <p className="text-[#c8c8c8]">{t("media.empty")}</p>
        ) : (
          media.map((m) => (
            <div key={m.id} className="flex flex-wrap sm:flex-nowrap items-center gap-3 border border-white/10 bg-[#1A1A1A] px-4 py-3">
              <span className="text-[9px] uppercase tracking-widest border border-[#D8CA82]/40 text-[#D8CA82] px-1.5 py-0.5 shrink-0">{t(`media.type.${m.type}`)}</span>
              <div className="flex-1 min-w-[140px]">
                <p className="text-sm font-semibold text-[#f7f7f7] break-words">{m.title}</p>
                <p className="text-xs text-[#c8c8c8] break-words">{m.game}{m.playerTag ? ` · ${m.playerTag}` : ""}{m.event ? ` · ${m.event}` : ""}</p>
              </div>
              <ConfirmDelete
                className="ml-auto"
                testId={`admin-media-delete-${m.id}`}
                itemLabel={`le média « ${m.title} »`}
                onConfirm={() => del(m.id)}
